...
tool_exec <- function(in_params, out_params)
{
  ...
  if(!is.null(quant_variables)) quant_variables <- clean_string(quant_variables)
  qual_variables <- in_params[[3]]
  if(!is.null(qual_variables)) qual_variables <- clean_string(qual_variables)
  quanti_sup <- in_params[[4]]
  if(!is.null(quanti_sup)) quanti_sup <- clean_string(quanti_sup)
  quali_sup <- in_params[[5]]
  if(!is.null(quali_sup)) quali_sup <- clean_string(quali_sup)
  ...
  ### Set up OUTPUT PATH for derived parameters
  env <- arc.env()
  wkspath <- env$scratchWorkspace
  out_pdf <- file.path(wkspath,"Plots.pdf")	
  out_txt <- file.path(wkspath,"Diagnostics.txt")
  ...
  out_params[[1]] <- out_pdf
  out_params[[2]] <- out_txt
